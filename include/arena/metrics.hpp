...
template <>
struct formatter<std::atomic<uint64_t>> : formatter<uint64_t>
{
    // format atomic<uint64_t> as uint64_t
    auto format(const std::atomic<uint64_t>& data, format_context& ctx) const noexcept -> decltype(ctx.out());
};
...
[[gnu::always_inline]] inline void metrics_probe_on_arena_reset([[maybe_unused]] arena::Arena* arena,
...
[[gnu::always_inline]] inline auto metrics_probe_on_arena_destruction([[maybe_unused]] arena::Arena* arena, void* cookie,
